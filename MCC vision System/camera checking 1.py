...
def capture_image_and_save(client_socket, directory, imageSaving):
    ...
    imgbytesSend = cv2.imencode('.png', cv2.resize(frameShow, (800,600)))[1].tobytes()  # ditto
...
while True:
    ...
    if TCPEnable:
        # Cek apakah executor sedang mengelola thread
        if executor._work_queue.qsize() == 0:
            # Mulai thread baru hanya jika executor sedang tidak mengelola thread
            future = executor.submit(
                receive_response, client_socket, directory, isSaving)
